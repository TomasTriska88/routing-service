...
if "parent_root_live =" not in s:
    marker = "          {% if states('switch.starlink_router') != 'on'\n"
    parent_block = '''          {% if states('switch.rodicovsky_rozvadec_zasuvka_1') != 'on'
                or states('select.rodicovsky_rozvadec_chovani_pri_zapnuti') != 'power_on' %}
            {% set ns.items = ns.items + ['Rodičovský rozvaděč'] %}
          {% endif %}
'''
    assert s.count(marker) == 1, "parent root insertion anchor mismatch"
    s = s.replace(marker, parent_block + marker, 1)
