...
has("20260821-advance-month-r1", "current advance-month energy version marker");
assert.strictEqual((src.match(/20260821-advance-month-r1/g) || []).length, 2);
has('Math.max(0, siteMonthEnergy) * price + Math.max(0, fixedMonthly)', "month-to-date all-in total includes full fixed monthly charge once");
has('Math.max(0, projectedMonthEnergy) * price + Math.max(0, fixedMonthly)', "month projection includes full fixed monthly charge once");
has('advance_entity: "input_number.elektrina_zaloha_mesic"', "headline cash-flow baseline comes from HA advance helper");
has('const financeHeadlineLabel = a.finance.overAdvance', "headline distinguishes advance from higher forecast");
has('class="history history-smart"');
has('class="smart-sharebar"');
has('class="monthline"');
has('class="finance-split"');
has('${this._esc(financeReferenceText)}', "headline shows advance or higher projected all-in amount");
has('${this._esc(financeHeadlineLabel)} · ${this._esc(financeHeadlineDetail)}', "headline explains whether amount is advance or forecast");
...
assert(render.indexOf('class="monthline"') < render.indexOf('class="finance-split"'));
assert(render.indexOf('class="finance-split"') < render.indexOf('Největší spotřebiče právě teď'));
