...
must((text.match(/20260821-advance-month-r1/g) || []).length === 2, "new version marker must appear twice");
...
must(text.includes("Tomáš ${this._fmt(smartTomasKwh, 1)} kWh"), "Tomáš kWh must be visible with full label");
must(text.includes("rodiče ${this._fmt(smartParentsKwh, 1)} kWh"), "parent kWh must be visible with full label");
...
must(text.includes('class="finance-split"'), "monthly proportional money split must render");
must(text.includes("${this._esc(financeReferenceText)}"), "advance-or-higher-forecast headline must render");
must(text.includes("${this._esc(financeHeadlineLabel)} · ${this._esc(financeHeadlineDetail)}"), "headline source detail must render");
must(text.includes("zahrnují společný měsíční fix právě jednou"), "all-in semantics must remain explicit without redundant visible label");
must(!text.includes("· vč. fixu"), "redundant visible fixed-charge suffix must stay removed");
...
for (const forbidden of [
  ...
  "<b>T ${this._fmt(smartTomasPct",
  "<b>R ${this._fmt(smartParentsPct",
]) {
  must(!history.includes(forbidden), `displayed ratio must never depend on stale/duplicate semantics: ${forbidden}`);
}
must(history.includes("<b>${this._fmt(smartTomasPct, 0)} %</b>"), "Tomáš percent must remain without redundant T prefix");
must(history.includes("<b>${this._fmt(smartParentsPct, 0)} %</b>"), "parent percent must remain without redundant R prefix");
